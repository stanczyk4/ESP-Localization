# Original Code Author: Pedro Gomes
# Source: http://stackoverflow.com/questions/2851233/how-can-i-retrieve-the-signal-strength-of-nearby-wireless-lan-networks-on-window
# Will be rewritten 
...
import distance
import filterNetworks as filter
...
def get_BSSI():
    ...
    try:
        ...
        # find each available network for each interface
        # Iterate through each interface. Possibly only one per run.
        for iface in ifaces:
            ...
            try:
                ...
                # Iterate through each network that was found
                for network in networks2:

                    # Get SSID
                    SSID = str(network.dot11Ssid.SSID[:network.dot11Ssid.SSIDLength])

                    # Look for all networks that begin with 'SPS' --> SubPos nodes
                    if filter.filterSSID(SSID):
                        continue
                    ...
                    # If the signal strength is less than -60dBm, sensitivity dictates this network is most likely not useful. Igonore.
                    if filter.filterSignalStrength(int(signal_strength)):
                        continue
